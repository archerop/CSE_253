...
def build_datasets(args: argparse.Namespace):
    ...
    if getattr(args, "use_cache", False):
        train_cache_dir = option4_cache_dir(args.subset_name, "train")
        val_cache_dir = option4_cache_dir(args.subset_name, "validation")

        train_dataset = CachedOption4MidiToAudioDataset(cache_dir=train_cache_dir)
        ...
        if args.overfit_n_samples is not None:
            ...
            return small, small, train_cache_dir, train_cache_dir

        val_dataset = CachedOption4MidiToAudioDataset(cache_dir=val_cache_dir)
        return train_dataset, val_dataset, train_cache_dir, val_cache_dir
...
def get_monitor_value(
    ...
    raise ValueError(f"monitor must start with train_ or val_, got {monitor}")
...
def plot_loss_curve(history_df: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("ContourNet-lite U-Net training curve")
    ax.legend()
...
def maybe_print_baseline_comparison(
    ...
    print()
    print("ContourNet-lite validation:")
    print(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Step 7: train ContourNet-lite U-Net main model."
    )

    parser.add_argument("--subset-name", type=str, default="debug")
    parser.add_argument("--use-cache", action="store_true", help="Use precomputed cached tensors.")
    ...
    tag = f"contour_unet_{args.subset_name}_{args.loss_mode}"
    if args.overfit_n_samples is not None:
        tag += f"_overfit{args.overfit_n_samples}"

    print("=" * 80)
    print("Step 7: ContourNet-lite U-Net main model")
    print("=" * 80)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_improvement(
            ...
            print(
                f"  no improvement: current={current_monitor_value:.6f}, "
                f"best={best_monitor_value:.6f}, "
                f"bad_epochs={epochs_without_improvement}/{args.early_stop_patience}"
            )
    ...
    save_prediction_figure(
        ...
        args=args,
        use_amp=use_amp,
        title=f"ContourNet-lite prediction example ({args.subset_name}, {args.loss_mode})",
    )
