...
def save_checkpoint(
    ...
    torch.save(
        {
            "model_name": "MelProjectedResidualCNN",
...
def plot_loss_curve(history_df: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("Mel-projected Residual CNN training curve")
    ax.legend()
...
def maybe_print_comparison(subset_name: str, loss_mode: str, val_metrics: Dict[str, float]) -> None:
    ...
    unet_path = OPTION4_OUTPUT_DIR / "metrics" / f"contour_unet_{subset_name}_{loss_mode}_eval.csv"

    print()
    print("Mel-projected Residual CNN validation:")
    print(
    ...
    if unet_path.exists():
        print()
        print("ContourNet-lite U-Net result on same subset/loss:")
        df = pd.read_csv(unet_path)
        cols = ["model", "logmel_l1", "logmel_mse", "energy_l1", "onset_l1", "loss"]
        print(df[cols].to_string(index=False))
    else:
        print()
        print(f"No U-Net eval found yet: {unet_path}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train mel-projected Residual CNN learned baseline for Option 4."
    )
    ...
    tag = f"mel_residual_cnn_{args.subset_name}_{args.loss_mode}"
    if args.overfit_n_samples is not None:
        tag += f"_overfit{args.overfit_n_samples}"

    print("=" * 80)
    print("Step 9A: Mel-projected Residual CNN learned baseline")
    print("=" * 80)
    ...
    eval_row = {
        "model": "mel_projected_residual_cnn",
    ...
    save_prediction_figure(
        ...
        args=args,
        use_amp=use_amp,
        title=f"Mel-projected Residual CNN prediction example ({args.subset_name}, {args.loss_mode})",
    )
